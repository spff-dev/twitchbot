...
module.exports = {
  name: 'test_sub',
  aliases: ['tsub'],
  description: 'Simulate a new sub announcement. Usage: !test_sub [tier{Prime|1|2|3}] [name]',
  ...
  async run(ctx) {
    ...
    const tier = tierLabel(ctx.args[0]);
    ...
    if (res.status !== 204) ctx.reply(`sub test failed (${res.status})`);
  }
};
